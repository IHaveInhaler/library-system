    ...
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <div className="relative flex h-full w-full max-w-md flex-col bg-white shadow-2xl dark:bg-gray-800">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {tab === 'copies' && (
            <ShelfCopiesTab shelf={shelf} copiesData={copiesData} />
          )}
        </div>
      </div>
    </div>
...
function ShelfCopiesTab({ shelf, copiesData }: { shelf: Shelf; copiesData: any }) {
  const qc = useQueryClient()
  ...
  const { data: bookResults } = useQuery({
    ...
    enabled: adding && bookSearch.length >= 2,
  })
  ...
  const createCopy = useMutation({
    ...
    onSuccess: () => {
      ...
      setAdding(false)
      setBookSearch('')
      qc.invalidateQueries({ queryKey: ['copies', 'shelf', shelf.id] })
    },
    onError: (err) => toast.error(extractError(err)),
  })
  ...
  return (
    <div className="space-y-3">
      ...
      {adding && (
        <div className="space-y-2 rounded-lg border border-dashed border-gray-300 p-3 dark:border-gray-600">
          ...
          {bookResults && bookResults.data.length > 0 && (
            <ul className="max-h-48 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700">
              {bookResults.data.map((book) => (
                <li key={book.id}>
                  <button
                    onClick={() => createCopy.mutate(book.id)}
                    disabled={createCopy.isPending}
                    className="flex w-full items-center gap-3 px-3 py-2 text-left text-sm hover:bg-gray-50 dark:hover:bg-gray-700/40"
                  >
                    {book.coverUrl ? (
                      <img src={book.coverUrl} alt="" className="h-8 w-6 flex-shrink-0 rounded object-cover" />
                    ) : (
                      <div className="flex h-8 w-6 flex-shrink-0 items-center justify-center rounded bg-gray-100 dark:bg-gray-700">
          ...
          <Button size="sm" variant="secondary" onClick={() => { setAdding(false); setBookSearch('') }}>Cancel</Button>
        </div>
      )}

      {!copiesData ? <PageSpinner /> : copiesData.data.length === 0 && !adding ? (
        <div className="rounded-lg border border-dashed border-gray-200 py-8 text-center dark:border-gray-700">
      ...
      ) : copiesData && copiesData.data.length > 0 && (
        <div className="space-y-2">
