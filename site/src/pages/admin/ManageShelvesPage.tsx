...
import { Layers, Plus, Search, X, Trash2, MapPin } from 'lucide-react'
...
function CreateShelfModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      ...
      onClose()
      setForm({ code: '', libraryId: '', genre: 'FICTION', position: 'L', location: '', capacity: '100' })
    },
    ...
    <Modal open={open} onClose={onClose} title="Create Shelf">
      <div className="space-y-4">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={() => create.mutate()} loading={create.isPending} disabled={!form.code || !form.libraryId}>
            Create Shelf
          </Button>
        </div>
      </div>
...
function EditShelfDrawer({ shelf, onClose }: { shelf: Shelf; onClose: () => void }) {
  ...
  const update = useMutation({
    mutationFn: () => shelvesApi.update(shelf.id, {
      ...form,
      capacity: parseInt(form.capacity, 10) || shelf.capacity,
      location: form.location || undefined,
    } as any),
      ...
      <div className="relative flex h-full w-full max-w-md flex-col bg-white shadow-2xl dark:bg-gray-800">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
          ...
          <div className="rounded-lg border border-gray-100 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800/50">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">Created: {new Date(shelf.createdAt).toLocaleDateString()}</p>
          </div>
          ...
          <div className="border-t border-gray-100 pt-4 dark:border-gray-700">
            <Button
              ...
              onClick={() => {
                if (confirm(`Delete shelf ${shelf.code}? This cannot be undone.`)) remove.mutate()
              }}
...
export default function ManageShelvesPage() {
  ...
  // Client-side search filter (shelves API doesn't have search param)
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      ...
      <div className="mb-4 flex flex-wrap gap-3">
        ...
        <select
          value={libraryId}
          onChange={(e) => setFilter('library', e.target.value)}
          className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
        >
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {filteredShelves.map((shelf) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{POSITIONS.find((p) => p.value === shelf.position)?.label ?? shelf.position}</td>
                    <td className="px-4 py-3">
                      {shelf.location ? (
